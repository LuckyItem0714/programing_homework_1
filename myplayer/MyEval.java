...
import ap25.*;
//ToDo: BitBoardを使ってpositionalValue(), score()の高速化
class MyEval implements Eval{// 盤面の評価を行うクラス。各マスに重みを与え、石の配置に基づいてスコアを計算します。
	...
	public int value(BitBoard board) {// ゲーム終了時はスコアに大きな重みをかけて返す。それ以外は、複合評価関数でスコアを計算。
		...
		/*} else {
			if(stoneCount < 7) {
				...
				w4 = 2;
				w5 = -2;
			}else if(stoneCount < 13) {
				w1 = 4; // 盤面の重み
				w2 = 6; // 自分の合法手の数
				w3 = -8; // 相手の合法手の数
				w4 = 2; // 自分の石の数
				w5 = -2; // 相手の石の数
			}else if (stoneCount < 25) {
				w1 = 3; // 盤面の重み
				w2 = 5; // 自分の合法手の数
				w3 = -7; // 相手の合法手の数
				w4 = 4; // 自分の石の数
