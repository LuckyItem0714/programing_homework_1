...
public class MyGame2 {
  public static void main(String args[]) {
    ...
    for (int i = 0; i < totalGames; i++) {
      Color myColor = BLACK;
      var player1 = new MyPlayer(myColor);
      var player2 = new RandomPlayer(myColor.flipped());
      var board = new MyBoard();
      var game = new MyGame(board, player1, player2);
      game.play();

      if (board.winner() == myColor) {
        myPlayerWins++;
      }
    }
    System.out.println(myPlayerWins);
    // 先攻時 87-10-3 勝率87%
    // 後攻時 95-3-2 勝率95%
    // 全体 182-13-5 勝率91%
  }
  ...
  public MyGame2(Board board, Player black, Player white) {// ゲームの初期化。盤面をコピーし、プレイヤーを登録。
   ...
   * 
   * プレイヤーの手が妥当かを検証。
   * エラーが発生した場合 → Move.ofError
   * 時間切れ → Move.ofTimeout
   * 合法手でない → Move.ofIllegal
   * 妥当な手ならそのまま返す。
   * 
   */
  ...
  public Color getWinnerColor(Board board) {
    return board.winner(); // NONE, BLACK, WHITE のいずれか
  }

  /*
   * 
   * 勝敗とスコアを表示。
   * 引き分けなら "draw"、勝者がいれば "○○ won by △△"。
   * 対戦カードと手の履歴も表示。
   * 
   */
  public void printResult(Board board, List<Move> moves) {
    ...
    if (score > 0)
      ...
      Color resultcolor=getWinnerColor(board);
    System.out.println(getWinner(board).toString());
